...
#define api_check_return(CHECK) if (!(CHECK)) return;
#define SHA256_DIGEST_SIZE (SHA256_DIGEST_LENGTH)

typedef SHA256_CTX sha256_t;

static void sha256_init (sha256_t *ss)
{
    SHA256_Init(ss);
}

static void sha256_update (sha256_t *ss, const uint8_t *key, size_t key_len)
{
    SHA256_Update(ss, key, key_len);
}

static void sha256_final (sha256_t *ss, uint8_t *kh)
{
    SHA256_Final(kh, ss);
}
